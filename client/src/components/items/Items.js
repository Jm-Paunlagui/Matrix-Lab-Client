...
export function ItemsPerPage({current_page, total_pages, items, total_items, has_prev, setDatas, Datas, page_number,has_next, moreClasses, children}) {
    ...
    return (
        <div className={`flex flex-col justify-end w-full ${moreClasses} p-4 space-y-2 lg:flex-row lg:space-x-2 lg:space-y-0 bg-blue-50 rounded-lg shadow`}>
        ...
        <button
          ...
          disabled={!has_prev}
          onClick={() =>
            setDatas({ ...Datas, page_number: page_number - 1 })
          }
          type="button"
        >
          <FontAwesomeIcon
