...
export default function OverallDashboard() {
    ...
    httpClient
      ...
      .then((response) => {
        ...
        setAnalysis({
          ...
          image_path_wordcloud_positive:
            response.data.image_path_wordcloud_positive,
          image_path_wordcloud_negative:
            response.data.image_path_wordcloud_negative,
        });
      });
  };
  ...
  const optionsForPVS = () => {
    httpClient
      ...
      .then((response) => {
        loadPolarityVsentiment(
          response.data.school_year[0].school_year,
          response.data.school_semester[0].school_semester,
          response.data.csv_question[0].csv_question,
        );
        setOptions({
          ...options,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 md:gap-6 pb-8">
            <div className="flex flex-col items-start w-full p-4 bg-blue-50 rounded-lg shadow space-y-2">
              <div className="w-full lg:w-3/5">
                <h1 className="text-md font-bold text-blue-500">Sentiments</h1>
              </div>
              {overall_sentiments.map((overall) => (
                <div
                  className={`flex flex-col items-start justify-start w-full p-4 rounded-lg ${overall.color50}`}
                  key={overall.id}
                >
                  <div className="flex items-center justify-center">
                    <div
            ...
            <div className="flex flex-col items-start w-full p-4 bg-blue-50 rounded-lg shadow space-y-2">
              ...
              <div className="flex flex-col items-start justify-start w-full p-4 rounded-lg">
                <img
                  alt="wordcloud"
                  className=""
                  src={`data:image/jpeg;base64,${image_path_wordcloud_positive}`}
                />
              </div>
            </div>
            <div className="flex flex-col items-start w-full p-4 bg-blue-50 rounded-lg shadow space-y-2">
              ...
              <div className="flex flex-col items-start justify-start w-full p-4 rounded-lg">
                <img
                  alt="wordcloud"
                  className=""
                  src={`data:image/jpeg;base64,${image_path_wordcloud_negative}`}
                />
              </div>
            </div>
          </div>
