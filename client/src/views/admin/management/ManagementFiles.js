...
import {faCaretLeft, faCaretRight, faTrash, faFileCsv} from "@fortawesome/free-solid-svg-icons";
...
export default function ManagementFiles() {
    ...
    const { loading, files, current_page, has_next, has_prev, items_per_page, next_page, prev_page, page, total_items, total_pages } = fileData;
    ...
    const loadFiles = (page) => {
        ...
        httpClient.get(`/data/list-of-csv-files-to-view/${page}`).then((response) => {
            setFileData({
                ...fileData,
                loading: false,
      ...
      <div className="px-6 mx-auto max-w-7xl">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 py-8 md:grid-cols-2 lg:grid-cols-3 gap-y-6 md:gap-6">
                {files.map((file) => (
                  <div className="flex flex-col mb-8 w-full bg-white rounded shadow border-solid border-4 border-blue-100" key={file.id}>
                      <div className="grid w-full h-full grid-cols-1 rounded">
                        <div className="col-span-1 w-full bg-blue-50">
                            <div className="flex flex-row w-full p-4">
                                ...
                                <h1 className="text-xl leading-none tracking-tight text-gray-700 ml-2">{file.id}</h1>
                            </div>
                        </div>
          ...
          <div className="mb-16 flex flex-col space-y-2 justify-end w-full lg:flex-row lg:space-x-2 lg:space-y-0">
              ...
              {/*    Page details*/}
                    <div className="flex flex-row items-center justify-center w-full">
                        <h1 className="text-base font-medium leading-none tracking-tight text-gray-700">
                            Showing {files.length} of {total_items} files in total ({total_pages} pages)
                        </h1>
                    </div>
              </div>
              <button
